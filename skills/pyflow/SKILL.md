---
name: pyflow
description: Build data flows using Pyflow Python DSL shorthand. This is the preferred approach for building data integration flows.
---
...
**No fan-out.** A Frame variable may only be consumed once. Joins/unions consume two chains, but each chain must be built independently. Self-joins: declare the table via two separate `q.source()` calls.
